...
export function ExploreGrid() {
  return (
    <section className="relative z-[2] border-t border-brd px-5 py-[100px] sm:px-8 lg:px-14">
          ...
          {EXPLORE_CARDS.map((card) => (
            <TransitionLink
              ...
              className={cn(
                ...
                card.tint
                  ? "border-brd-2 bg-gradient-to-br from-accent/16 to-transparent hover:from-accent/22"
            ...
            >
              ...
              />
              <div className="mb-3.5 font-mono text-[11px] tracking-[.2em] text-accent">
                {card.eyebrow}
              </div>
              <div className="mb-2.5 text-[19px] font-semibold">{card.title}</div>
              <div className="mb-4 text-sm leading-[1.6] text-muted">{card.description}</div>
              <div className="text-[13px] text-accent-2">{card.cta}</div>
            </TransitionLink>
          ))}
        </RevealGroup>
      </div>
